...
// Program to display complete URL
  ...
  $link .= $_SERVER['PHP_SELF'];
       ...
       $url= "http://localhost/onlineShopping/category.php?category=".$_GET['category']."";
        ...
        $queryCheck = mysqli_query($conn, "SELECT category_name FROM kidit_category WHERE category_id=".$_SESSION['category']."");
        $getCategoryName = mysqli_fetch_assoc($queryCheck);
?>

    <header>
      <div >
        <span class=text style="color: red;padding: 14px 0px 14px 25px;"><a href="index.php" style="color:red;">Home</a> / <a href="shop.php" style="color:red;">Shop</a> / <a href="category.php?category=<?php echo $_GET['category']; ?>" style="color:red">Category - <?php echo $getCategoryName['category_name']; ?></a></span>
         <p class="message"></p>
      ...
      <div class="dropDown-wrapper">
        ...
        {
            $categorySelected = "category_id = ".$_SESSION['category']." AND";
            $sort = " ";
        }

        if($_SESSION['sort'] && $_SESSION['category'])
        {
            if($_SESSION['sort'] == 1)
            {
              $sort = "ORDER BY product_name ASC";
   ...
   <div class="container">
    <div class="row">
                <?php
                    $qryProduct = mysqli_query($conn, "SELECT * FROM kidit_product WHERE $categorySelected product_status = 1 $sort");
